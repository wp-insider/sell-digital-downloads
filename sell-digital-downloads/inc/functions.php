...
function isell_absolute_from_url($src_file_url) {
    ...
    if (preg_match("/^http/i", $src_file_url) != 1)
        return FALSE;    // Not a qualified URL.
    ...
    if ($domain_name_pos === FALSE)
        return FALSE;            // Rats!  URL is not on this server.
...
function isell_generate_order_title($order_id = NULL, $args = NULL, $title_format = NULL) {
    ...
    if ($order_id == NULL) {
        ...
        $title = sprintf(
                $title_format, $product_name, $txn_id, $payment_status, $amount_paid, $buyer_name, $buyer_email
        );
...
if (!function_exists('isell_settings_page_view')) {
    ...
    function isell_settings_page_view() {
        ...
        if (isset($_POST['submit']) && isset($_POST['isell_options_page'])) {
            if (!wp_verify_nonce($_POST['nonce'], 'isell_options_page'))
                return;
